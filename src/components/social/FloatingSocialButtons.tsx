...
export default function FloatingSocialButtons() {
  ...
  if (!whatsappChannel && !telegramChannel) return null;

  const buttons = [
    whatsappChannel && {
      name: 'WhatsApp',
      url: whatsappChannel.channel_url,
      color: '#25D366',
      icon: (
    ...
    telegramChannel && {
      name: 'Telegram',
      url: telegramChannel.channel_url,
      color: '#0088cc',
        ...
        {isOpen && (
          ...
          >
            ...
            <motion.a
              href="/help"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0, transition: { delay: buttons.length * 0.1 } }}
              exit={{ opacity: 0, x: 20 }}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              className="flex items-center gap-3 px-4 py-3 rounded-full bg-primary text-primary-foreground shadow-lg"
            >
